...
const TankActor = require('../client/TankActor')
...
class MyRenderer extends Renderer {
    ...
    draw() {
        ...
        for (let objId of Object.keys(this.sprites)) {
            ...
            if (objData) {
                ...
                if ((sprite !== this.Tank) && sprite.actor) {
                    sprite.actor.thrustEmitter.emit = !!objData.showThrust;
                }

                if (objData.class == Tank && sprite != this.playerTank) {
                ...
                if (objData.class == Tank){
                    sprite.actor.tankContainerSprite.rotation = this.gameEngine.world.objects[objId].angle * Math.PI/180;
                } else{
                ...
                if (sprite != this.playerTank && viewportSeesLeftBound && objData.position.x > this.viewportWidth - this.camera.x) {
                    sprite.x = objData.position.x - worldWidth;
                }
                if (sprite != this.playerTank && viewportSeesRightBound && objData.position.x < -this.camera.x) {
                    sprite.x = objData.position.x + worldWidth;
                }
                if (sprite != this.playerTank && viewportSeesTopBound && objData.position.y > this.viewportHeight - this.camera.y) {
                    sprite.y = objData.position.y - worldHeight;
                }
                if (sprite != this.playerTank && viewportSeesBottomBound && objData.position.y < -this.camera.y) {
                    sprite.y = objData.position.y + worldHeight;
                }
            }
    ...
    addObject(objData, options) {
        ...
        if (objData.class == Tank) {
            ...
            if (this.clientEngine.isOwnedByPlayer(objData)) {
                this.playerTank= sprite; // save reference to the player ship
                sprite.actor.tankSprite.tint = 0XFF00FF; // color  player ship
    ...
    updateOffscreenIndicator(objData){
        ...
        let playerTankObj = this.gameEngine.world.objects[this.playerTank.id];
        let slope = (objData.position.y - playerTankObj.position.y) / (objData.position.x -playerTankObj.position.x);
        ...
        if (objData.position.y <playerTankObj.position.y - this.viewportHeight/2) {
            indicatorPos.x = this.viewportWidth/2 + (padding - b)/slope;
            indicatorPos.y = padding;
        } else if (objData.position.y > playerTankObj.position.y + this.viewportHeight/2) {
            indicatorPos.x = this.viewportWidth/2 + (this.viewportHeight - padding - b)/slope;
            indicatorPos.y = this.viewportHeight - padding;
        }

        if (objData.position.x < playerTankObj.position.x - this.viewportWidth/2) {
            indicatorPos.x = padding;
            indicatorPos.y = slope * (-this.viewportWidth/2 + padding) + b;
        } else if (objData.position.x > playerTankObj.position.x + this.viewportWidth/2) {
        ...
        if (indicatorPos.x == 0 && indicatorPos.y == 0){
            ...
            let rotation = Math.atan2(objData.position.y - playerTankObj.position.y, objData.position.x - playerTankObj.position.x);
    ...
    updateScore(data){
        ...
        for (let id of Object.keys(data)){
            ...
            if (scoreEl == null){
                ...
                if (this.playerTank && this.playerTank.id == parseInt(id)) scoreEl.classList.add('you');
    ...
    onKeyChange(e){
        if (this.playerTank) {
            if (e.keyName === 'up') {
                // this.playerTank.actor.thrustEmitter.emit = e.isDown;
            }
        }
    }
...
function getCentroid(objects) {
    ...
    for (let id of Object.keys(objects)){
        ...
        if (obj.class == Tank) {
            ...
            let objDistance = Math.sqrt( Math.pow((selectedTank.position.x-obj.position.y), 2) + Math.pow((selectedTank.position.y-obj.position.y), 2));
            if (selectedTank== obj || objDistance < maxDistance) {
    ...
    centroid.x /= tankCount;
    centroid.y /= tankCount;
